 ...
 */
 ...
 void get_raw_data_from_file(std::string Filename, float *&xyz_array, unsigned char *&rgb_array, unsigned long& size, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &Cloud){
...
void my_zfp_compression_xyz(void *&buffer, unsigned int org_size, float *xyz, size_t& bytes_size, double tolerance){
    ...
    //cout << "write header size = " << tem_size << endl;
...
void my_zfp_decompression_xyz(void* buffer, size_t bufsize, float *&xyz){
    ...
    cout << "read header size = " << tem_size << endl;

    //if(field->nx == 0) field->nx = 1;
    //if(field->ny == 0) field->ny = 1;
    //if(field->nz == 0) field->nz = 1;
